...
const GalaxyBackground = () => {
  ...
  const nebulaRotate = useTransform(smoothProgress, [0, 1], [0, 30]);
  const elementsY = useTransform(smoothProgress, [0, 1], [0, -150]);
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[-1] overflow-hidden bg-[#020205]">
      ...
      >
        {/* Primary Nebula */}
        <div className="absolute top-[-20%] left-[-10%] w-[80%] h-[80%] rounded-full bg-brand-primary/10 blur-[150px] animate-pulse" />
        <div className="absolute bottom-[-20%] right-[-10%] w-[80%] h-[80%] rounded-full bg-blue-600/10 blur-[150px] animate-pulse" style={{ animationDelay: '2s' }} />
        
        {/* Secondary Accents */}
        <div className="absolute top-[20%] right-[15%] w-[40%] h-[40%] rounded-full bg-purple-600/5 blur-[120px]" />
        <div className="absolute middle-0 left-[30%] w-[30%] h-[30%] rounded-full bg-cyan-500/5 blur-[100px]" />
      </motion.div>

      {/* Orbital Rings (Solar System Feel) */}
      <motion.div 
        style={{ rotate: nebulaRotate }}
      ...
      {floatingElements.map((el, i) => (
        ...
        >
          <div 
            ...
            style={{ 
              boxShadow: `0 0 20px ${el.color}20`,
              color: el.color 
            }}
          >
            <el.Icon size={32} strokeWidth={1.5} className="drop-shadow-[0_0_8px_currentColor]" />
